...
def table_tennis_game():
    ...
    gameOver = False
    score = [0, 0]
    botPos = [59, 200]  # initial position of bot

    def moveBot(ballPos, botPos):
        if ballPos[1] < botPos[1]:
            botPos[1] -= 5
        elif ballPos[1] > botPos[1]:
            botPos[1] += 5
        botPos[1] = np.clip(botPos[1], 20, 415)
        return botPos
    ...
    while True:
        ...
        if gameOver and solo:
            img = imgGameOver
            cv2.putText(img, str(score[0] + score[1]).zfill(2), (565, 375), cv2.FONT_HERSHEY_COMPLEX,
                        2.5, (200, 0, 200), 5)
        elif gameOver:
            img = imgGameOver
